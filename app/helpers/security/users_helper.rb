module Security::UsersHelper
  ...
  def build_user_form(action,submit_caption,is_edit = nil)
    ...
    field_configs << {:field_type   => 'PasswordField',
                      :field_name   => 'password',
                      :non_db_field => true}
    ...
    field_configs << {:field_type => 'DropDownField',
                      ...
                      :settings   => {:list => Branch.for_select}}
  ...
  def build_users_tree(users)
    begin
      ...
      menu2.add_command("Quick Add programs",url_for(:action => "quick_add_programs"))
      menu2.add_command("Quick Permissions Settings ",url_for(:action => "quick_permissions_setting"))
      menu2.add_command("Quick Export Permissions ",url_for(:action => "quick_export_permissions"))
      #MM112015 - quick adding of programs
  ...
  #MM112015 - quick adding of programs
  def build_quick_add_program_form(action,caption)

    session[:quick_add_program_form]= Hash.new
    session[:quick_add_program_form][:user_id] = @user_id

    field_configs = []
    field_configs << {:field_type => 'Screen',
                      :field_name => "add_programs",
                      :settings   => {:target_action => 'add_programs',
                                      :id_value      => @user_id,
                                      :width         => 2000,
                                      :height        => 800,
                                      :no_scroll     => true
  ...
  def build_programs_list_grid(data_set,can_edit,can_delete)

    column_configs = []

    column_configs << {:field_type => 'text',:field_name => 'id', :hide => true, :col_width => 50}
    column_configs << {:field_type => 'text',:field_name => 'functional_area_name', :hide => true, :col_width => 160}#, :col_width => 160
    column_configs << {:field_type => 'text',:field_name => 'program_name', :col_width => 300}#, :col_width => 1090
    column_configs << {:field_type => 'text', :field_name => 'display_name', :column_caption => 'Display name', :column_width => 300}
    column_configs << {:field_type => 'text', :field_name => 'is_non_web_program', :data_type => 'boolean', :column_caption => 'Is non web program', :col_width => 100}
    column_configs << {:field_type => 'text', :field_name => 'disabled', :data_type => 'boolean', :column_caption => 'disabled', :col_width => 100}
    column_configs << {:field_type => 'text', :field_name => 'is_leaf', :data_type => 'boolean', :column_caption => 'Is leaf', :col_width => 100}
    column_configs << {:field_type => 'text', :field_name => 'is_mf', :data_type => 'boolean', :column_caption => 'Is mf', :col_width => 100}

    @multi_select = "selected_programs"
  ...
  def build_quick_permissions_setting_form(action,caption)

    session[:quick_permissions_setting_form]= Hash.new
    session[:quick_permissions_setting_form][:user_id] = @user_id

    field_configs = []
    field_configs << {:field_type => 'Screen',
                      :field_name => "add_permissions_settings",
                      :settings   => {:target_action => 'add_permissions_settings',
                                      :id_value      => @user_id,
                                      :width         => 2000,
                                      :height        => 800,
                                      :no_scroll     => true
    ...
    build_form(nil,field_configs,action,'program_permissions_setting',caption,false)
  end
  ...
  def build_programs_permissions_settings_list_grid(data_set, can_edit, can_delete=nil, can_cancel=nil,multi_select=nil)
    ...
    column_configs << {:field_type => 'text',:field_name => 'id', :hide => true, :col_width => 50}
    column_configs << {:field_type => 'text',:field_name => 'functional_area_name', :hide => true, :col_width => 160}
    column_configs << {:field_type => 'text',:field_name => 'program_name', :col_width => 250}
    column_configs << {:field_type => 'text',:field_name => 'display_name', :column_caption => 'Display name', :column_width => 250}
    column_configs << {:field_type => 'text',:field_name => 'security_group_name',:editor => :select, :select_options =>security_groups, :col_width => 200}
    column_configs << {:field_type => 'text',:field_name => 'is_non_web_program', :data_type => 'boolean', :column_caption => 'Is non web program', :col_width => 100}
    column_configs << {:field_type => 'text',:field_name => 'disabled', :data_type => 'boolean', :column_caption => 'disabled', :col_width => 100}
    column_configs << {:field_type => 'text',:field_name => 'is_leaf', :data_type => 'boolean', :column_caption => 'Is leaf', :col_width => 100}
    column_configs << {:field_type => 'text',:field_name => 'is_mf', :data_type => 'boolean', :column_caption => 'Is mf', :col_width => 100}
    ...
    get_data_grid(data_set, column_configs, nil,true, nil, :save_action => '/security/users/update_programs_permissions_settings')

  end

  def build_export_user_permissions_form(user,action,caption)

    session[:quick_export_permissions_form]= Hash.new
    session[:quick_export_permissions_form][:user_id] = user.id

    field_configs = []
    field_configs << {:field_type => 'Screen',
                      :field_name => "add_export_permissions",
                      :settings   => {:target_action => 'add_export_permissions',
                                      :id_value      => user.id,
                                      :width         => 2000,
                                      :height        => 800,
                                      :no_scroll     => true
    ...
    build_form(nil,field_configs,action,'export_permissions',caption,false)
  end

  def build_export_permissions_list_grid(data_set,can_edit,can_delete)
    ...
    column_configs << {:field_type => 'text',:field_name => 'program_name', :col_width => 300}#, :col_width => 1090
    column_configs << {:field_type => 'text',:field_name => 'display_name', :column_caption => 'Display name', :column_width => 300}
    column_configs << {:field_type => 'text',:field_name => 'security_group_name', :column_caption => 'security group name',:col_width => 200}
    column_configs << {:field_type => 'text',:field_name => 'is_non_web_program', :data_type => 'boolean', :column_caption => 'Is non web program', :col_width => 100}
    column_configs << {:field_type => 'text',:field_name => 'disabled', :data_type => 'boolean', :column_caption => 'disabled', :col_width => 100}
    column_configs << {:field_type => 'text',:field_name => 'is_leaf', :data_type => 'boolean', :column_caption => 'Is leaf', :col_width => 100}
    column_configs << {:field_type => 'text',:field_name => 'is_mf', :data_type => 'boolean', :column_caption => 'Is mf', :col_width => 100}

    @multi_select = "selected_export_permissions"
  ...
  def build_new_user_form(user,action,caption,is_edit = nil,is_create_retry = nil)
    ...
    build_form(user,field_configs,action,'user',caption,is_edit)
  ...
  def build_user_grid(data_set,can_edit,can_delete)
    ...
    column_configs[6] = {:field_type => 'text',:field_name => 'hashed_password'}
    #  ----------------------
