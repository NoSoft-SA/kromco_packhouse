class UpdateRunStats
  ...
  def update_run_stats(jobs)
    ...
    jobs.group_by { |j| j['production_run_id'] }.each do |run_id, jobs_for_run|
      cartons_printed = 0
      cartons_weight = 0
      rebins_printed = 0
      rebins_weight = 0
      ...
      query = "update production_run_stats set cartons_printed = cartons_printed +  #{cartons_printed},
