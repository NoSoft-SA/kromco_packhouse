...
def remove_program
  ...
  puts "prog user is nil: " + prog_user.to_s
...
def add_program_submit
 ...
 if ! FunctionalArea.exists_for_user?(user.id,func_area_name)
    ...
    puts "new func id: " + @func_area_node_id
    @tree_name = "users"
  end
...
def get_password
  ...
  puts "password is: " + password.to_s
...
def add_user
  ...
  def find_users_submit
   begin
	...
	puts "users is nil: " + (@users == nil).to_s
	if @users.length == 0
...
def export
    if session[:program_user_export].has_key?("locally")
        ...
        prog_user = ProgramUser.find(prog_user_id)
        puts prog_user.program_id.to_s
        prog_name = prog_user.program.program_name
        func_area_name = prog_user.program.functional_area.functional_area_name
...
def export_entire_permissions
  begin
      ...
      @usernames = User.find_by_sql("select * from users order by user_name asc").map{|g| [g.user_name]}
