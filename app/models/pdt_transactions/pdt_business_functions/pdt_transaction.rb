class PDTTransaction
  ...
  def build_choice_screen(prompt_msg_array=nil,screen_attributes=nil,plugins=nil)
    ...
    if prompt_msg_array.class.name == "Array"
      index = prompt_msg_array.length - 1
...
#   begin
    ...
    if params['mode'] != PdtScreenDefinition.const_get("CONTROL_VALUE_REPLACE").to_s
    ...
    @jsession_store.set_input_pdt_screen_def(input_pdt_screen_def) if params['mode']!= PdtScreenDefinition.const_get("CONTROL_VALUE_REPLACE").to_s
    ...
    @jsession_store.set_result_screen(result_screen) if params['mode'] != PdtScreenDefinition.const_get("CONTROL_VALUE_REPLACE").to_s
    @jsession_store.set_active_transaction(self) if @new_transation_activated == false && params['mode'] != PdtScreenDefinition.const_get("CONTROL_VALUE_REPLACE").to_s
    #   @env = nil
...
#     begin
    ...
    if ! (error = validate_method(method_to_route_to))
      ...
      if  @object_to_route_to.pdt_screen_def.mode.to_s == PdtScreenDefinition.const_get("CONTROL_VALUE_REPLACE").to_s
        return "<error>#{method_to_route_to} NOT FOUND</error>"
      else
