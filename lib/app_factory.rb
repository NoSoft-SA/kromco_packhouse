...
#    self.slice(0..start_pos-1).reverse.each {|a| yield a }
...
class ViewSettings
  ...
  def gen_grid_code
    ...
    @code_lines.push "  action_configs = []"
    lines = Array.new
    ...
    field_def = "    action_configs << {:field_type => 'action',:field_name => 'edit " + @model_settings.model_var_name + "',\n"
    ...
    field_def = "    action_configs << {:field_type => 'action',:field_name => 'delete " + @model_settings.model_var_name + "',\n"
    ...
    lines.push "  #action_configs << {:field_type => 'separator'} if can_edit || can_delete\n"
    lines.push "  column_configs << {:field_type => 'action_collection', :field_name => 'actions', :settings => {:actions => action_configs}} unless action_configs.empty?\n"
      ...
      if field_name!= "id"
        ...
        data_type = case column[:type]
                    ...
                    when 'int', 'int4'
                      ", :data_type => 'integer'"
                    when 'numeric'
                      ", :data_type => 'number', :format => 'delimited_1000'"
                    else
                      nil
                    end
  ...
  def build_#{@model_settings.model_var_name}_dm_grid(data_set, stat, columns_list, can_edit, can_delete, grid_configs)
    ...
    # ----------------------
    if can_edit
      action_configs << {:field_type => 'action',:field_name => 'edit #{@model_settings.model_var_name}',
        :column_caption => 'Edit',
        :settings =>
       {:link_text => 'edit',
        :link_icon => 'edit',
    ...
    if can_delete
      action_configs << {:field_type => 'action',:field_name => 'delete #{@model_settings.model_var_name}',
...
EOC
  ...
  def gen_form_code
   ...
   begin
      ...
      #construct_form(golfer,field_configs,"'" + action + "'",'golfer',action,is_edit)

      @code_lines.push "  construct_form(" + @model_settings.model_var_name + ",field_configs," +
 ...
 #'construct_form' method of the application helper- which will generate the actual HTML
...
EOC
 ...
 def gen_dynamic_search_combos_handlers
  ...
  def generate_create_entity_code
     ...
     lines.push "  # store_list_as_grid_url # UNCOMMENT if this action is called directly (i.e. from a menu, not from a grid)"
  ...
  def generate_submit_entity_search
     ...
     lines.push "  store_last_grid_url"
...
#     lines.push " begin"
     ...
     lines.push "     flash[:notice] = 'new record created successfully'"
     lines.push "     redirect_to_last_grid"
     lines.push "  else"
...
#-----------------------------------------------------------------------------------
 ...
 def get_association_def(column_def,assoc_table)
  ...
  class YamlMaker
    ...
    # Wrap long line of text. Taken from actionview/lib/action_view/helpers/text_helper.rb, line 223 (Rails 4.1.4)
    ...
    def self.make_yml_report_string( for_class, group_name=nil )
      ...
      sels         = []
      wheres       = []
      grid_configs = {'column_widths' => {}, 'data_types' => {}, 'column_captions' => {}, 'formats' => {}}
      ...
      column_names.each do |k|
        ...
        grid_configs['formats'][v.name] = 'delimited_1000' if ['amount', 'price'].any? {|k| v.name.include?(k) }
      ...
      unless grid_configs['formats'].empty?
        grid_str << "  formats:\n"
        grid_configs['formats'].each {|k,v| grid_str << "    # #{k}: #{v}\n" }
      end
      grid_str << <<EOS
...
EOS
      ...
      sel_text   = word_wrap("#{column_names.map {|c|"#{tn}.#{c}"}.join(', ')}#{xtra_sel}").gsub("\n", "\n  ")
      where_text = word_wrap(where_clause).gsub("\n", "\n  ")
      sql        = "SELECT #{sel_text} \n  FROM #{tn} #{join_str} \n  WHERE(#{where_text})"
    ...
    def self.make_yml_report_without_model(table_name, group_name)
      cols = PostgresMetaData.get_column_defs(table_name, ActiveRecord::Base.connection)
      ...
      cols.each do |col|
        ...
        if ['float8', 'numeric'].include?(col[:type])
          grid_configs['data_types'][col[:field_name]] = 'number'
        end
        if 'int4' == col[:type]
          grid_configs['data_types'][col[:field_name]] = 'integer'
        end
        if 'bool' == col[:type]
          grid_configs['data_types'][col[:field_name]] = 'boolean'
        end
        if 'date' == col[:type]
          grid_configs['data_types'][col[:field_name]] = 'date'
        end
        grid_configs['column_captions'][col[:field_name]] = Inflector.humanize(col[:field_name])
        #grid_configs['formats'][col[:field_name]] = 'delimited_1000' if ['amount', 'price'].any? {|k| v.name.include?(k) }
        grid_configs['formats'][col[:field_name]] = 'delimited_1000' if ['float8', 'numeric'].include?(col[:type])
      end


      grid_str = "grid_configs:\n  caption: #{table_name.humanize}\n  column_widths:\n  data_types:\n"
      grid_configs['data_types'].each {|k,v| grid_str << "    #{k}: #{v}\n" }
      grid_str << "  column_captions:\n"
      grid_configs['column_captions'].each {|k,v| grid_str << "    #{k}: #{v}\n" }
...
EOS
      ...
      sel_text = word_wrap(column_names.map {|c|"#{table_name}.#{c}"}.join(', ')).gsub("\n", "\n  ")
      sql      = "SELECT #{sel_text} \n  FROM #{table_name} \n  WHERE(true)"
      grp_name = group_name.nil? || group_name == 'None' ? nil : "default_report_index_group_name: #{group_name}\n"
  ...
  # A small utility for generating code that can be copied and pasted.
  # Takes a model and returns create statement using attributes.
  # Call like this: script/runner "AppFactory::CodeGenerator.new.statement('User')"
  class CodeGenerator

    def statement(for_class)
      model_class  = for_class.constantize
      column_names = model_class.column_names.dup
      column_names = column_names.delete_if {|c| c == 'created_at' || c == 'updated_at' || c == 'id' }
      cols         = column_names.map {|c| ":#{c} => ''"}.join(",\n      ")
      s =<<-EOS
res = #{for_class}.create!(
      #{cols})
      EOS
      puts s
    end
