...
#	============================
	...
	validates_presence_of :season_code
	validates_numericality_of :season
#	=====================
#	 Complex validations:
#	=====================
def validate 
...
def validate_uniqueness
	 exists = Season.find_by_season_code_and_commodity_code(self.season_code,self.commodity_code)
	 if exists != nil 
		errors.add_to_base("There already exists a record with the combined values of fields: 'season_code' and 'commodity_code' ")
	end
end
#	===========================
...
def self.get_season_codes_for_commodity_code(commodity_code)
    season_codes = Season.find_by_sql("select distinct season_code from seasons where commodity_code = '#{commodity_code}'").map{|g| [g.season_code]}
