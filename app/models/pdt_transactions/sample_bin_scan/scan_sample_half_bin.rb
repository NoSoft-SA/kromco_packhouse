class ScanSampleHalfBin < PDTTransactionState
 ...
 def Scan_sample_half_bin
   if(self.is_screen_request)
     return build_default_screen
   elsif(self.pdt_screen_def.mode.to_s == PdtScreenDefinition.const_get("ENTERDATA").to_s)
     Scan_sample_half_bin_submit()
   end
 end

 def Scan_sample_half_bin_submit
   self.current_bin_id = self.pdt_screen_def.controls[self.pdt_screen_def.controls.length-1]["value"]

  self.parent.number_of_half_bins_scanned = self.parent.number_of_half_bins_scanned + 1
