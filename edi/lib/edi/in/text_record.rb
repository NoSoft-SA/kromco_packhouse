  ...
  def extract_fields
    ...
    @field_descriptors.each do |config|
      begin
        ...
        nullify_zeros = config.attribute('nullify_zeros').to_s

        value = next_field_value(@cursor_pos,size)

        raise EdiValidationError, "value required for field: " + config.to_s if required == "true" && (value == nil ||value.strip() == "")
       ...
       if value.class.to_s == "String"
        ...
        if(nullify_zeros && nullify_zeros == "true" && value.to_s.strip == "0")
          value = ""
        end
