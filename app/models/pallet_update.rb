class PalletUpdate
  ...
  def org_changed?(carton)
    if old_carton = Carton.find_by_carton_number(carton.carton_number)
      if carton.organization_code != old_carton.organization_code
        return true
      end
    end
  ...
  def set_carton_fg_data (carton, org_change = nil,is_not_repack = nil)
    ...
    if org_change || ! is_not_repack
      carton.organization_code = repr_carton.organization_code
      carton.derive_puc_account
    end
  ...
  def store_representative_carton(carton,production_run_code,is_not_repack = nil)
    ...
    if @puc_groups[production_run_code][:group_num]== 1
      ...
      #-------------------------------------------
      if ! is_not_repack||org_change
        puts "updating fg on all groups"
        @puc_groups.each do |key,puc_group|
          puc_group[:cartons].each do |ctn|
            set_carton_fg_data(ctn,org_change,is_not_repack)
          end
        end
      end
    end
  ...
  def update_group(representative_carton,puc_group)
    ...
    fields_not_to_copy = ["ppecb_inspection_id","shift_id","egap","carton_fruit_nett_mass_actual","date_time_created","date_time_erp_xmit","exit_date_time","rw_create_datetime","track_indicator_code","erp_cultivar","carton_number","carton_id","rw_receipt_carton_id","production_run_id","production_run_code","farm_code","puc","account_code","line_code","shift_code","carton_label_station_code","erp_station","erp_pack_point","carton_pack_station_code","is_inspection_carton","qc_datetime_out","qc_datetime_in","pick_reference","iso_week_code","pack_date_time","pc_code","qc_status_code","qc_result_status","packer_number","bin_id"]
